...
async fn postgres_test<F, T>(future: F) -> T
where
...
async fn create_test_pool() -> sqlx::PgPool {
    let database_url = "postgres://postgres:postgres@localhost:5432/simple-db-test";
    ...
    sqlx::query("DROP SEQUENCE IF EXISTS users_id_seq")
        .execute(&pool)
        .await
        .expect("Failed to drop users_id_seq sequence");
...
#[tokio::test]
async fn test_postgres_driver_insert() {
    postgres_test(async {
...
#[tokio::test]
async fn test_postgres_driver_find_all() {
    postgres_test(async {
        let pool = create_test_pool().await;
...
#[tokio::test]
async fn test_postgres_driver_filter() {
    postgres_test(async {
        let pool = create_test_pool().await;
        let driver = PostgresDriver::new(pool.clone());
...
#[tokio::test]
async fn test_postgres_driver_update() {
    postgres_test(async {
        let pool = create_test_pool().await;
        let driver = PostgresDriver::new(pool.clone());
    ...
    let age = row.get_by_name("age")
        .and_then(|v| v.as_i64())
        .or_else(|| row.get_by_name("age").and_then(|v| v.as_i32().map(|i| i as i64)));
    assert_eq!(age, Some(26));
    }).await;
}

#[tokio::test]
async fn test_postgres_driver_delete() {
    postgres_test(async {
        let pool = create_test_pool().await;
        let driver = PostgresDriver::new(pool.clone());
...
#[tokio::test]
async fn test_postgres_driver_transaction_commit() {
    postgres_test(async {
        let pool = create_test_pool().await;
        let driver = PostgresDriver::new(pool.clone());
...
#[tokio::test]
async fn test_postgres_driver_transaction_rollback() {
    postgres_test(async {
        let pool = create_test_pool().await;
        let driver = PostgresDriver::new(pool.clone());
...
#[tokio::test]
async fn test_postgres_driver_ping() {
    postgres_test(async {
        let pool = create_test_pool().await;
        let driver = PostgresDriver::new(pool);
...
#[tokio::test]
async fn test_postgres_driver_data_types() {
    postgres_test(async {
        let pool = create_test_pool().await;

        sqlx::query("DROP TABLE IF EXISTS test_types CASCADE")
            .execute(&pool)
            .await
            .expect("Failed to drop test_types table");

        sqlx::query("DROP SEQUENCE IF EXISTS test_types_id_seq")
            .execute(&pool)
            .await
            .expect("Failed to drop test_types_id_seq sequence");
    ...
    let int_val = row.get_by_name("int_val")
        .and_then(|v| v.as_i64())
        .or_else(|| row.get_by_name("int_val").and_then(|v| v.as_i32().map(|i| i as i64)));
