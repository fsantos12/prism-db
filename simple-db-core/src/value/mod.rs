...
pub(super) const CATEGORY_INLINE: u64 = 0b0000000000000000;

/// Category: boxed value (stores heap-allocated pointer in payload).
pub(super) const CATEGORY_BOXED:  u64 = 0b0100000000000000;
...
impl DbValue {
    ...
    ///
    /// # Safety
    /// Assumes x86-64 canonical 48-bit user-space pointers.
    #[inline]
    pub(super) fn from_tag_and_boxed<T>(tag: u64, val: T) -> Self {
    ...
    #[inline]
    pub(super) fn category(&self) -> u64 {
        self.tag() & CATEGORY_MASK
    }

    /// Extracts the type identifier from the tag.
    #[inline]
    pub(super) fn ty(&self) -> u64 {
        self.tag() & TYPE_MASK
    }
    ...
    #[inline]
    pub(super) unsafe fn payload_as_ref<T>(&self) -> &T {
    ...
    pub fn as_char(&self) -> Option<char> {
        if self.ty() != TYPE_CHAR { return None; }
    ...
    /// Creates a signed 64-bit integer (inline if fits in 48 bits, else boxed).
    #[inline]
    ...
    pub fn as_i64(&self) -> Option<i64> {
        if self.ty() != TYPE_I64 { return None; }
        match self.category() {
    ...
    /// Creates an unsigned 64-bit integer (inline if fits in 48 bits, else boxed).
    #[inline]
    ...
    pub fn as_f64(&self) -> Option<f64> {
        if self.ty() != TYPE_F64 || self.category() != CATEGORY_BOXED { return None; }
    ...
    pub fn as_string(&self) -> Option<&str> {
        if self.ty() != TYPE_STRING || self.category() != CATEGORY_BOXED { return None; }
    ...
    pub fn as_bytes(&self) -> Option<&[u8]> {
        if self.ty() != TYPE_BYTES || self.category() != CATEGORY_BOXED { return None; }
    ...
    /// Returns a human-readable string describing the current value's type.
    #[inline]
...
// =============================================================================
// Trait impls — memory management, cloning, comparison
// =============================================================================
...
/// Equality comparison — types must match; comparison is by value, not bits.
...
// =============================================================================
// Conversion macros — used by convert.rs
// =============================================================================

/// `impl From<T> for DbValue` for a single type.
macro_rules! impl_from_t {
    ($t:ty, $constructor:ident) => {
        impl From<$t> for DbValue {
            #[inline]
            fn from(v: $t) -> Self { Self::$constructor(v) }
        }
    };
}

/// `impl From<Option<T>> for DbValue` — maps `None` to `DbValue::from_null()`.
...
/// `impl TryFrom<&DbValue> for T` and `impl TryFrom<DbValue> for T`.
///
/// Two variants:
/// - `copy`  — type is `Copy`, extracted by value
/// - `clone` — type requires cloning
macro_rules! impl_try_from {
    ...
    ($t:ty, $as_fn:ident, $type_name:expr, clone) => {
        impl TryFrom<&DbValue> for $t {
            ...
            #[inline]
            fn try_from(value: &DbValue) -> Result<Self, Self::Error> {
                value.$as_fn().map(|v| v.clone()).ok_or_else(|| TypeError::Mismatch {
                    expected: $type_name.to_string(),
                    found:    value.type_name().to_string(),
                }.into())
            }
